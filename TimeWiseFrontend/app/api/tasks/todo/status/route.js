...
export async function POST(req) {
  try {
    ...
    const response = await fetch(
      `${process.env.BACKEND_SERVER_URL}/api/tasks/todo/status`,
      {
        method: 'POST',
        headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorText || 'Failed to modify task todo status.' },
    ...
    console.error('Task todo status modification Error:', error);
    return NextResponse.json(

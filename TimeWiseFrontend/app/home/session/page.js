...
const scenarioOptions = [
  ...
  },
  {
    id: 4,
    name: "Zen Garden",
    description: "Traditional Working bells",
...
const SessionPage = () => {
  ...
  const [currentTimeWiseSession, setcurrentTimeWiseSession] = useState(null);
  const [sessionForm, setSessionForm] = useState({
    duration: 30,
    sessionGoal: "",
  });
  const [newTask, setNewTask] = useState("");
  const [userPrompt, setUserPrompt] = useState(""); // For AI session generation
  ...
  useEffect(() => {
    ...
    const savedSession = localStorage.getItem("currentTimeWiseSession");
    if (savedSession) {
      const parsed = JSON.parse(savedSession);
      setcurrentTimeWiseSession(parsed);
    ...
    const handleBeforeUnload = (e) => {
      if (currentTimeWiseSession && isRunning) {
        e.preventDefault();
        e.returnValue = "";
      }
    };
  ...
  useEffect(() => {
    if (currentTimeWiseSession) {
      localStorage.setItem(
        "currentTimeWiseSession",
        JSON.stringify({
          ...currentTimeWiseSession,
          timeRemaining,
          isPaused,
  ...
  const handleNavigationConfirm = async (shouldSave) => {
    if (shouldSave) {
      ...
      localStorage.removeItem("currentTimeWiseSession");
    } else {
      localStorage.removeItem("currentTimeWiseSession");
      setcurrentTimeWiseSession(null);
    }
    ...
    <AlertDialog open={showExitDialog} onOpenChange={setShowExitDialog}>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Would you like to save your current Working session?
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
  ...
  const handleStartSession = () => {
    ...
    const newSession = {
      sessionCreator: userData ? userData.userName : null, 
      sessionTimeStamp: new Date(),
      duration: sessionForm.duration,
      sessionGoal: sessionForm.sessionGoal,
    };
    
    setcurrentTimeWiseSession(newSession);
    setTimeRemaining(sessionForm.duration * 60);
    setIsRunning(true);
    setIsSoundPlaying(true);
  };

  // New function to generate session from AI
  const handleGenerateSession = async () => {
    setIsGenerating(true);
    try {
      const userData = JSON.parse(localStorage.getItem('TimeWiseUserData'));
      if (!userData || !userData.userTasks) {
        alert("No tasks found. Please add tasks to your account first.");
        setIsGenerating(false);
        return;
      }
  
      // Build URL with query parameter
      const url = `/api/session/generate${userPrompt ? `?userPrompt=${encodeURIComponent(userPrompt)}` : ''}`;
      
      // Send only the tasks array in the request body
      const response = await fetch(url, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(userData.userTasks),
      });
      ...
      if (response.ok) {
        ...
        setSessionForm({
          duration: generatedSession.duration || 30,
          sessionGoal: generatedSession.sessionGoal || ""
        });
        
        // Update tasks from AI suggestion
        if (generatedSession.tasksOperated && generatedSession.tasksOperated.length > 0) {
          const tasksSet = new Set(generatedSession.tasksOperated);
          setCompletionForm(prev => ({
            ...prev,
            tasksOperated: tasksSet,
            sessionOutcome: generatedSession.sessionOutcome || ""
          }));
        }
      ...
      alert("An error occurred while generating session");
    } finally {
      setIsGenerating(false);
    }
  };
  ...
  const handleSaveSession = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setUserPrompt("");
        window.location.href = "/home/session/previous";
      }
    } catch (error) {
  ...
  useEffect(() => {handleGenerateSession();},[]);
      ...
      <Card>
        <CardHeader>
          ...
          <div className="flex justify-between items-center">
            {/* Left Side: Title */}
            <CardTitle>
              <p className="text-3xl font-bold">Working Session</p>
            </CardTitle>
          ...
          {!currentTimeWiseSession &&
          <CardDescription>
            {showCompletionForm ? "Complete your session" : "Start a new session"}
          </CardDescription>
          }
        </CardHeader>
        <CardContent>
          {!currentTimeWiseSession && !showCompletionForm ? (
            <div className="space-y-4">
              ...
              {/* New AI Session Generation Section */}
              <div className="border p-4 rounded-lg bg-black">
                <div className="flex items-center mb-2">
                  <Sparkles className="h-5 w-5 text-primary mr-2" />
                  <Label htmlFor="userPrompt" className="font-medium">Session Generator</Label>
                </div>
                <textarea
                  id="userPrompt"
                  placeholder="Tell what kind of session you want (e.g., 'I need to focus on marketing tasks' or 'Create a session for deep work on coding')"
                  value={userPrompt}
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  Generates a session based on your tasks and preferences
                </p>
              </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea
                  id="goal"
                  placeholder="What was your session goal?"
                  value={currentTimeWiseSession?.sessionGoal || ""}
                  onChange={(e) => 
                    setcurrentTimeWiseSession(prev => ({
                      ...prev,
                      sessionGoal: e.target.value
                    }))
                  }
                  ...
                  <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-sm text-muted-foreground">
                              {currentTimeWiseSession?.sessionGoal}
                            </span>
                          </div>
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-sm text-muted-foreground">
                              {currentTimeWiseSession?.duration} minutes
                            </span>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-sm font-medium">
                            ...
                            <span className="text-sm text-muted-foreground">
                              {format(
                                new Date(currentTimeWiseSession?.sessionTimeStamp),
                                "pp"
                              )}
                            </span>
                          </div>
                    ...
                    <Card>
                      <CardHeader>
                        ...
                        <CardDescription>
                          Capture thoughts during your session
                        </CardDescription>
                      </CardHeader>
                      <CardContent>
                        <textarea
        ...
        <CardFooter>
          <p className="text-xs text-center w-full text-muted-foreground">
            {currentTimeWiseSession
              ? "Working sessions help you track your time and stay focused on your goals."
              : "Start a new Working session to boost your productivity."}
          </p>
        </CardFooter>
      </Card>

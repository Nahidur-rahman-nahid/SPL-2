    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="members" className="w-full">
        <TabsList className="grid w-full grid-cols-3">
          ...
          <TabsTrigger value="requests">Requests ({requestedToJoinMembers.length})</TabsTrigger>
          <TabsTrigger value="invited">Invited ({invitedMembers.length})</TabsTrigger>
        </TabsList>
...
const fetchTeamDetails = async () => {
  ...
  const handleRemoveMember = async (memberName) => {
    setActionLoading(true);
    try {
      const response = await fetch(`/api/teams/user/remove?teamName=${encodeURIComponent(team.teamName)}&memberName=${encodeURIComponent(memberName)}`, {
        method: 'DELETE',
    })
  ...
  const AddTaskDialog = ({ open, onClose, onAdd, teamTasks }) => {
    const [searchTerm, setSearchTerm] = useState('');
    ...
    useEffect(() => {
      ...
      if (userData?.userTasks && currentUser) {
        const filteredTasks = userData.userTasks
          ...
          .filter((taskName) => !teamTasks.includes(taskName));
    ...
    const handleSubmit = () => {
      if (isCustom && customTask.trim()) {
        onAdd(customTask.trim());
        onClose();
    ...
    const handleCreateNewClick = () => {
      router.push("/home/task/create");
      setIsCustom(true); 
    };
  
    return (
      <Dialog open={open} onOpenChange={onClose}>
        <DialogContent className="sm:max-w-sm w-[90%] top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] fixed">
          <DialogHeader>
      ...
      Create New
              ...
              <>
                ...
                <Select value={selectedTask} onValueChange={setSelectedTask}>
                  ...
                  <SelectContent className="max-h-[200px]"> {/* Adjusted max-h */}
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={(isCustom && !customTask.trim()) || (!isCustom && !selectedTask)}
            >
              Add Task
            </Button>
          </DialogFooter>
  ...
  const handleAcceptRequest = async (memberName) => {
    setActionLoading(true);
    try {
      const responseValue='Accept';
      const response = await fetch(`/api/teams/request/response?teamName=${encodeURIComponent(team.teamName)}&respondedTo=${encodeURIComponent(memberName)}&response=${encodeURIComponent(responseValue)}`, {
        method: 'PUT',
    })
  ...
  const handleRemoveTask = async (taskName) => {
    setActionLoading(true);
    try {
      const response = await fetch('/api/teams/task/remove', {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  // handleAddTask and handleRemoveTask implementation
const handleAddTask = () => {
  ...
  const handleInviteMember = async (userName) => {
    ...
    setActionLoading(true);
    try {
        const response = await fetch(`/api/teams/invite?teamName=${encodeURIComponent(team.teamName)}&recipient=${encodeURIComponent(userName)}`, {
          method: 'POST',
        });
  ...
  const addTaskToTeam = async ( taskName) => {
    try {
      const response = await fetch('/api/teams/task/add', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ teamName: team.teamName, taskName: taskName }),
      });
  
      if (!response.ok) {
        throw new Error('Failed to add task to team');
      }
      ...
      toast({
        title: 'Task added',
        description: `"${taskName}" has been added to the team.`,
        variant: 'default',
      });
    } catch (error) {
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <Button variant="ghost" onClick={() => router.push('/home/team')}>
            ...
            <AnalyzeDataButton data={team}
      buttonText="Analyze Team Details"/>
          <ActionMenu />
        </div>
